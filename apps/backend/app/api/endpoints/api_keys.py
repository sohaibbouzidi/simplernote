...
def create_api_key(key_in: APIKeyCreateSchema, db: Session = Depends(get_db), current_user=Depends(AuthService.get_current_user)):
    ...
    ActivityLogService.record(db, user_id=str(current_user.id), action="create", entity_type="api_key", entity_id=str(api_key["id"]), project_id=str(api_key["project_id"]), payload={"name": api_key["name"]}, auth_method=getattr(current_user, '_auth_method', 'user'))
...
def revoke_api_key(key_id: str, db: Session = Depends(get_db), current_user=Depends(AuthService.get_current_user)):
    ...
    ActivityLogService.record(db, user_id=str(current_user.id), action="delete", entity_type="api_key", entity_id=key_id, project_id=str(api_key.project_id), payload={"name": api_key.name}, auth_method=getattr(current_user, '_auth_method', 'user'))
